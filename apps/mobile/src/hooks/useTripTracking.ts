...
/**
 * Estado e rastreamento de GPS da viagem, vivendo em `HomeTabs` (que nunca desmonta
 * enquanto o motorista está logado) em vez de em `TripScreen` (que desmontava a cada
 * troca de aba). Achado da auditoria de performance: `TripScreen` guardava o
 * `watchSubscription` num `useRef` próprio — trocar pra "Mensagens"/"Rota" no meio do
 * turno desmontava a tela, o cleanup do efeito chamava `watchSubscription.remove()`, e o
 * GPS parava de ser gravado silenciosamente até o motorista voltar pra aba "Viagem". Não
 * era só performance, era perda de dado de viagem.
 */
