...
/** Chamado pelo AuthContext para reagir a um 401 (access token expirado ou inválido) —
...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!res.ok) {
    // Sem refresh silencioso de propósito: o access token dura 15min
    // (`app.jwt.access-ttl-minutes`) e a sessão deve expirar de verdade nesse prazo, não
    // se renovar sozinha por até 30 dias via refresh token.
